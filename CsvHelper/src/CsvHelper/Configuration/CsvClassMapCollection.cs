...
namespace CsvHelper.Configuration
{
    ...
    public class CsvClassMapCollection
    {
        ...
        public virtual CsvClassMap this[Type type]
        {
            get
            {
                var map = this.data.Keys.Where(k => k.IsAssignableFrom(type)).Select(k => this.data[k]).FirstOrDefault();
        ...
        {
            var type = this.GetGenericCsvClassMapType(map.GetType()).GetGenericArguments().First();
        ...
        internal virtual void Remove(Type classMapType)
        {
            ...
            var type = this.GetGenericCsvClassMapType(classMapType).GetGenericArguments().First();
