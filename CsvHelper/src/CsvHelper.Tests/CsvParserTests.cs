...
namespace CsvHelper.Tests
{
    ...
    public class CsvParserTests
    {
        ...
        {
            var config = new CsvConfiguration { CountBytes = true, BufferSize = 4 };
        ...
        {
            var config = new CsvConfiguration { BufferSize = 12 };
            using (var stream = new MemoryStream())
            using (var writer = new StreamWriter(stream))
            using (var reader = new StreamReader(stream))
            using (var parser = new CsvParser(reader, config))
        ...
        public void DoNotIgnoreBlankLinesTest()
        {
            ...
            using (var parser = new CsvParser(reader))
            {
                ...
                row = parser.Read();
                Assert.AreEqual(string.Empty, row[0]);
                Assert.AreEqual(string.Empty, row[1]);
                Assert.AreEqual(string.Empty, row[2]);
