...
namespace CsvHelper.Tests
{
    ...
    public class CsvReaderTests
    {
        ...
        {
            // TODO: Implement me. You may need mocking for this test
        }

        [TestMethod]
        public void GetTypeTest()
        {
            var data = new[] { "1", "blah", DateTime.Now.ToString(), "true", "c", string.Empty, Guid.NewGuid().ToString() };
        ...
        {
            var headerData = new[] { "IntColumn", "String Column", "GuidColumn" };
            var recordData = new[] { "1", "string column", Guid.NewGuid().ToString() };
            var queue = new Queue<string[]>();
        ...
        {
            var headerData = new[] { "Column", "Column", "Column" };
        ...
        public void SkipEmptyRecordsTest()
        {
            ...
            queue.Enqueue(new[] { string.Empty, string.Empty, string.Empty });
        ...
        public void SkipEmptyRecordsObeysTrimFieldsIsEnabledTest()
        {
            var queue = new Queue<string[]>();
            queue.Enqueue(new[] { "1", "2", "3" });
            queue.Enqueue(new[] { " ", string.Empty, string.Empty });
            queue.Enqueue(new[] { "4", "5", "6" });
            queue.Enqueue(null);
        ...
        public void SkipRecordCallbackTest()
        {
            ...
            Assert.AreEqual(string.Empty, reader.CurrentRecord[1]);
            Assert.AreEqual(string.Empty, reader.CurrentRecord[2]);
        ...
        public void DoNotIgnoreBlankLinesTest()
        {
            ...
            using (var csv = new CsvReader(reader))
            {
                ...
                Assert.AreEqual(string.Empty, records[1].Name);
                Assert.AreEqual(null, records[2].Id);
                Assert.AreEqual(string.Empty, records[2].Name);
        ...
        private sealed class TestDefaultValuesMap : CsvClassMap<TestDefaultValues>
        {
            ...
            {
                this.Map(m => m.IntColumn).Default(-1);
                this.Map(m => m.StringColumn).Default(null);
            }
        }
        ...
        private sealed class TestRecordMap : CsvClassMap<TestRecord>
        {
            ...
            {
                this.Map(m => m.IntColumn).TypeConverter<Int32Converter>();
                this.Map(m => m.StringColumn).Name("String Column");
                this.Map(m => m.TypeConvertedColumn).Index(1).TypeConverter<TestTypeConverter>();
                this.Map(m => m.FirstColumn).Index(0);
                this.Map(m => m.GuidColumn);
                this.Map(m => m.NoMatchingFields);
            }
        }
        ...
        private sealed class TestRecordDuplicateHeaderNamesMap : CsvClassMap<TestRecordDuplicateHeaderNames>
        {
            ...
            {
                this.Map(m => m.Column1).Name("Column").NameIndex(0);
                this.Map(m => m.Column2).Name("Column").NameIndex(1);
                this.Map(m => m.Column3).Name("Column").NameIndex(2);
            }
        }
